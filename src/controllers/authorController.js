...
const createAuthor = async function (req, res) {
  try {
    ...
    if (
      authorData.title != 'Mr' && authorData.title != 'Mrs' && authorData.title != 'Miss'
    )
...
const loginAuthor = async function (req, res) {
  try {
    ...
    let findAuthor = await AuthorModel.findOne({email: userName,password: password,
    });
    if (!findAuthor)
      return res.status(404).send({ status: false,msg: 'Email or Password is not valid',});
